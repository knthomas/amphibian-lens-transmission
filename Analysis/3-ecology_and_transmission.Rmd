...
#create shape vector for source of data
sh_source_hab <- c("literature" = 2, 
               "this_study" = 17)

#create shape vector for source of data
sh_source_act <- c("literature" = 6, 
               "this_study" = 25)
...
boxplot_habs.t50 <- ggplot(data = lens_adults, 
                       aes(x = hab, y = t50, fill = hab)) + 
  geom_violin(trim=TRUE, alpha = 0.5, width = 1) +
  scale_fill_manual(values = col_hab) +
  geom_boxplot(width=0.2, fill="white", outlier.alpha = 0)+
  stat_summary(fun.y = mean, aes(fill = hab), geom = "point", shape = 23, size = 2.5, alpha = 0.5, show_guide = FALSE) +
 geom_jitter(aes(shape = source), size = 1, alpha = 0.9, position = position_jitter(0.12)) +
  ...
  theme(text = element_text(size=12), panel.background = element_blank(), axis.line = element_line(colour = "black"), legend.key = element_rect(fill = NA)) +
...
boxplot_act.t50 <- lens_adults %>%
  ...
  ggplot(aes(x = act, y = t50, fill = act)) + 
  geom_violin(trim=TRUE, alpha = 0.5, width = 1) +
  scale_fill_manual(values = col_act) +
  geom_boxplot(width=0.2, fill="white", outlier.alpha = 0)+
  stat_summary(fun.y = mean, aes(fill = act), geom = "point", shape = 23, size = 2.5, alpha = 0.5, show_guide = FALSE) +
 geom_jitter(aes(shape = source), size = 1, alpha = 0.9, position = position_jitter(0.12)) +
  scale_shape_manual(values = sh_source,
                     name = "Data",
                     breaks = c("this_study", "literature"),
...
# boxplot of %UVA across activity periods
boxplot_act.uva <- lens_adults %>%
  mutate(act = fct_relevel(act, "diurnal", "nondiurnal")) %>%
  ggplot(aes(x = act, y = pUVA, fill = act)) + 
  geom_violin(trim=TRUE, alpha = 0.5, width = 1) +
  scale_fill_manual(values = col_act) +
  geom_boxplot(width=0.2, fill="white", outlier.alpha = 0)+
  stat_summary(fun.y = mean, aes(fill = act), geom = "point", shape = 23, size = 2.5, alpha = 0.5, show_guide = FALSE) +
...
# boxplot of T50 across habs
pdf("../Figures/boxplots.pdf", width = 7, height = 9)
