...
col_ecocat <- c("both" = "#C107B6",
                "diurnal" = "#FFA010",
                "neither" = "#808080", #boxplot fig uses #636363 but with alpha it's lighter
                "scansorial" = "#009E73")
...
hylodes <- species_curves %>%
  ...
  mutate(uva_larger = ifelse(wavelength_nm<401 & wavelength_nm>314, Hylodes.phyllodes.larger, NA)) %>%
  mutate(uva_smaller = ifelse(wavelength_nm<401 & wavelength_nm>314, Hylodes.phyllodes.smaller, NA))
...
plot_hylodes <- ggplot(hylodes, aes(x = wavelength_nm)) + 
  ...
  geom_point(aes(filter(lens_hyl, stage=="adult")$t50, y=50), size = 3, col = "black", shape = 15) +
  geom_point(aes(filter(lens_hyl, stage=="adult_smaller")$t50, y=50), size = 3, col = "#989595", shape = 15) +
...
alytes <- species_curves %>%
  ...
  mutate(uva_adult = ifelse(wavelength_nm<401 & wavelength_nm>314, Alytes.muletensis.adult, NA)) %>%
  mutate(uva_tad = ifelse(wavelength_nm<401 & wavelength_nm>314, Alytes.muletensis.tadpole, NA))
...
plot_alytes <- ggplot(alytes, aes(x = wavelength_nm)) + 
  ...
  geom_path(aes(y = Alytes.muletensis.adult, color = "adult"), size = 1) +
   #plot T50 onto each curve
  geom_point(aes(filter(lens_dev, stage=="adult")$t50, y=50), size = 3, col = "black", shape = 15) +
  geom_point(aes(filter(lens_dev, stage=="tadpole")$t50, y=50), size = 3, col = "#989595", shape = 15) +
  #fill in area under curves between 305-400nm to show %UVA
  geom_area(aes(y = uva_adult), fill = "black", alpha = 0.4) +
  geom_area(aes(y = uva_tad), fill = "#989595", alpha = 0.4) +
...
dendro <- species_curves %>%
  ...
  mutate(uva_lecuo = ifelse(wavelength_nm<401 & wavelength_nm>314, Dendropsophus.leucophyllatus, NA)) %>%
  mutate(uva_minus = ifelse(wavelength_nm<401 & wavelength_nm>314, Dendropsophus.minusculus, NA)) %>%
  mutate(uva_tad = ifelse(wavelength_nm<401 & wavelength_nm>314, Dendropsophus.minutus.tadpole, NA))
...
plot_dendro <- ggplot(dendro, aes(x = wavelength_nm)) + 
  ...
  #fill in area under curves between 305-400nm to show %UVA
  geom_area(aes(y = uva_lecuo), fill = "red", alpha = 0.4) +
  geom_area(aes(y = uva_minus), fill = "black", alpha = 0.4) +
  geom_area(aes(y = uva_tad), fill = "#989595", alpha = 0.4) +
   #plot T50 onto each curve
  geom_point(aes(filter(lens_adults, genus_species=="Dendropsophus_leucophyllatus")$t50, y=50), size = 3, col = "red", shape = 15) +
  geom_point(aes(filter(lens_adults, genus_species=="Dendropsophus_minusculus")$t50, y=50), size = 3, col = "black", shape = 15) +
  geom_point(aes(filter(lens_tads, genus_species=="Dendropsophus_minutus")$t50, y=50), size = 3, col = "#989595", shape = 15) +
...
#export figure
pdf("../Figures/curves.pdf", width = 10, height = 3)
plot_grid(plots)
dev.off()
```
