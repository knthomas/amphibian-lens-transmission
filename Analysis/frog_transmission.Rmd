...
alytes <- species_curves %>%
  ...
  mutate(uva_adult = ifelse(wavelength_nm<401, Alytes.muletensis.adult, NA)) %>%
  mutate(uva_tad = ifelse(wavelength_nm<401, Alytes.muletensis.tadpole, NA))
...
dendro <- species_curves %>%
  ...
  mutate(uva_lecuo = ifelse(wavelength_nm<401, Dendropsophus.leucophyllatus, NA)) %>%
  mutate(uva_minus = ifelse(wavelength_nm<401, Dendropsophus.minusculus, NA)) %>%
  mutate(uva_tad = ifelse(wavelength_nm<401, Dendropsophus.minutus.tadpole, NA))
...
fig.a <- plot_hylodes + theme(legend.position=c(.8,.2))
fig.b <- plot_alytes + theme(legend.position=c(.8,.2))
fig.c <- plot_dendro + theme(legend.position=c(.8,.2))
...
#export figure
pdf("../Figures/curves.pdf", width = 15, height = 4)
plot_grid(plots)
dev.off()
```
