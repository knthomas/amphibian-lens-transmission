...
Species averages for T50 and %UVA, normalized to both 700nm (standard) and 450nm (to adjust for long-wavelength dropoff).
...
Ecological trait database imported here and matched to our species subset. In the original trait database used for other projects, Adult Habitat is categorized as: fossorial, subfossorial, aquatic, semiaquatic, ground-dwelling, scansorial; here, it is collapsed into binary states (scansorial or other) to test whether scansoriality is correlated with lens transmission. Likewise, in our original database, activity pattern was categorized as: diurnal, nocturnal, both; here, it would make sense to make it binary (regularly active during day or not) to test whether dirunal activity correlates with lens transmission. However, I think this necessitates going through each species originally assigned to the "both" category to see what makes sense. For example, something crepuscularly active might be categorized as "both", but not necessarily active in daylight, while something found out often at both noon and midnight would clearly be exposed to bright daylight. 

```{r trait-data}
#import trait data
traits_raw <- data.frame(read.csv("../Data/traits.csv", header=TRUE, na.strings=c("", "NA")))

#tidy database and extract relevant columns
traits <- traits_raw %>%
  mutate(genus_species = as.factor(paste(traits_raw$Genus, traits_raw$Species, sep="_"))) %>%
  distinct(genus_species, .keep_all = TRUE) %>% #remove any duplicates
  select(genus_species, Feng_tiplabel, Adult_habitat, Activity_period, Mating_habitat, Sex_dichromatism, Pupil_shape, Larval_habitat, Life_history) 

#convert to binary traits
traits_bin <- traits %>%
  ...
  select(genus_species, binary_hab, Activity_period, Sex_dichromatism) 
...
We additionally looked at what trait data we had for the species published in Yavonovich et al. 2020 (all species in their paper were current ASW names, checked 21 oct 2020)

```{r}

#import species data for lens transmission
lens_yav <- data.frame(read.csv("../Data/yovanovich_species.csv", header=TRUE, na.strings=c("", "NA"))) #load raw data
colnames(lens_yav) <- "genus_species"

#merge lens and trait data by species name
lens_traits_yav <- left_join(lens_yav, traits_bin, by="genus_species") 

#export
write.csv(lens_traits_yav, file = "lens_trait_yav.csv")
